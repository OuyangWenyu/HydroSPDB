...
from hydroDL.trainer import train_function
...
class MultitTaskTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        """
        with open(os.path.join(os.path.dirname(__file__), "test_configs/multi_decoder_test.json")) as a:
            cls.model_params = json.load(a)
        with open(os.path.join(os.path.dirname(__file__), "test_configs/multitask_decoder.json")) as a:
