...
class MyTestCaseGages(unittest.TestCase):
    def setUp(self) -> None:
        """Here we don't randomly select samples from the whole dataset in each batch iteration in an epoch.
        Just simply shuffle the whole dataset and pick them by the new shuffled order. We call this easier LSTM"""
        config_file = copy.deepcopy(cfg)
        args = cmd(sub="basic/exp38", quick_data=1, cache_state=1, mini_batch=[256, 365], train_epoch=50, save_epoch=2)
    ...
    def test_train_gages_easier_lstm(self):
