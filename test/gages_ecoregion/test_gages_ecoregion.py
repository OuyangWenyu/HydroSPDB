...
class MyTestCaseGages(unittest.TestCase):
    def setUp(self) -> None:
        # python gages_pub_analysis.py --sub ecoregion/exp1 --cache_state 1 --pub_plan 4 --plus 0 --split_num 2
        self.config_file = copy.deepcopy(cfg)
        args = cmd(sub="ecoregion/exp1", cache_state=1, pub_plan=4, plus=0, split_num=2)
        ...
        with torch.cuda.device(self.gpu_num):
            for i in range(self.split_num):
                ...
                basin_area_baseline = data_model_baseline.data_source.read_attr(
                    data_model_baseline.t_s_dict["sites_id"],
