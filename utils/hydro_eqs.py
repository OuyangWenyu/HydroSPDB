"""Typical equations in hydrology"""
import numpy as np
...
def penman_monteith(rad: np.array, hmd: np.array, e0: np.array, wnd: np.array, tmpr: np.array, elev: np.array,
                    tau: np.array, ref, option=0):
    ...
    (E) This function computes the reference ET using Penman Monteith method
     ...
     RET is in mm/day
     Parameters:
         rad: shortwave incident radiation (MJ/m2/day), unless option == 1 is specified
         hmd: relative humidity ([-])
         e0: saturation vapor pressure (equation available in makeDay, temp degrees C)
         wnd: wind speed m/s
         tmpr: temperature (celcius)
         elev: elevation (m)
         tau: air transmittance, but, when option ==1, this does not matter
         ref: a choice of reference crop. == 0 is FAO grass, == 1 is alfalfa
         option: when option is 1, rad means net radiation
    Return:
    ...
    # % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
    # ref = 2;
    if ref == 0:  # FAO reference grass, height 0.12, surface resistance 90 s / m, albedo=0.23
        c = (Delta + gamma * (1 + 0.34 * wnd))
        a1 = 0.408 * Delta / c
        a2 = gamma * DE * wnd / TK * 900. / c
        rET = a1 * Rn + a2
        ra = 208. / wnd  # ra of the grass reference
        rETs = (Rn * Delta + rho * gamma * DE / ra) / (
                lambda_ * (Delta + gamma))  # here rho is a combined term, not density
        ...
        rc = 49
        CT = 1710 - 6.85 * tmpr
        rET = (Delta * Rn / lambda_ + gamma * CT * DE / (ra * lambda_)) / (Delta + gamma * (1 + rc / ra))
        ...
        rc = 49
        # % CT = 1710 - 6.85 * T
        rET = (Delta * Rn + gamma * rho * DE / ra) / (lambda_ * (Delta + gamma * (1 + rc / ra)))
    elif ref == 3:
        ...
        rET = Delta / (Delta + gamma) * Rn / lambda_ + gamma / (Delta + gamma) * 6.430 * (
                1 + 0.536 * wnd) * DE / lambda_
    if ref < 3:
