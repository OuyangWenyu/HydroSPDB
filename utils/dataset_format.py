...
def trans_nldas_to_camels():
    """transform the format of forcing data generated by GEE to the one of CAMELS dataset"""
...
def trans_daymet_to_camels(daymet_dir, output_dir, gage_dict, region, year):
    ...
    for f_name in os.listdir(daymet_dir):
        if fnmatch.fnmatch(f_name, 'daymet_' + region + '_mean_' + str(year) + '.csv'):
            ...
            for i_basin in range(gage_dict['STAID'].size):
                ...
                output_file = os.path.join(output_huc_dir, gage_dict['STAID'][i_basin] + '_lump_daymet_forcing.txt')
