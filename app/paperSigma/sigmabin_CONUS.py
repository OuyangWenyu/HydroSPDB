...
if 'test' in doOpt:
    ...
    for k in range(0, len(hucLst)):
        ...
        statConf = ds.statCalConf(predField='LSTM', targetField='SMAP',rmBias=True)
...
if 'plotBin' in doOpt:
    ...
    for iHuc in range(0, len(hucLst)):
        ...
        for k in range(0, nbin):
            ...
            # distTemp = np.array([(conf-yRank).max(), (conf-yRank).min()])
            # dist = distTemp[np.argmax(np.abs(distTemp))]
