...
if 'train' in doLst:
    ...
    for k in dLst:
        sd = utils.hydro_time.t2dt(ty[0]) - dt.timedelta(days=k)
        ed = utils.hydro_time.t2dt(ty[1]) - dt.timedelta(days=k)
...
if 'test' in doLst:
    ...
    ypAnnLst = list()
    modelName = 'LSTM'
    model = train.model_load(outFolder, 100, modelName=modelName)
    yp = train.model_test(model, x, c, batchSize=100).squeeze()
    ypLstmLst.append(
    ...
    for k in dLst:
        ...
        yP = train.model_test(model, (x, obs), c, batchSize=100).squeeze()
        ypLstmLst.append(
            dbCsv.transNorm(
