...
import numpy as np
from refine import huc_single_test
import imp
import matplotlib
imp.reload(refine)
refine.reload()
...
if 'train' in doOpt:
    ...
    for kk in range(0, len(drLst)):
        for k in range(0, 18):
            trainName = 'hucn1_'+str(k+1).zfill(2)+'_v2f1'
            opt['train'] = trainName
            opt['dr'] = drLst[kk]
            opt['out'] = trainName+'_y15_Forcing_dr'+drStrLst[kk]
            cudaID = k % 3
            print(trainName)
...
if 'test' in doOpt:
    drLst = [0.2, 0.8]
    drStrLst = ["%02d" % (x*100) for x in drLst]
    for kk in range(0, len(drLst)+1):
        for k in range(0, 18):
            trainName = 'hucn1_'+str(k+1).zfill(2)+'_v2f1'
            if kk == len(drLst):
                outName = trainName+'_y15_Forcing'
            else:
                outName = trainName+'_y15_Forcing_dr'+drStrLst[kk]
            cudaID = kk % 3
            huc_single_test.runTestCmd(
                outName=outName, cudaID=cudaID, screenName='huc'+str(k+1).zfill(2))
