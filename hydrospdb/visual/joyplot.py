...
def joyplot(
    ...
    **kwds
            ...
            if column is not None:
                converted.append(
                    [
                        ...
                        if _is_numeric(g) and k in column
                    ]
                )
                if i == 0:
                ...
                converted.append(
                    [_remove_na(g) for k, g in group.items() if _is_numeric(g)]
                )
                if i == 0:
                    sublabels = [k for k, g in group.items() if _is_numeric(g)]
    #################################################################
    ...
    #################################################################
        ...
        if column is not None:
            converted = [
                [_remove_na(g)] for g in data if _is_numeric(g) and i in column
            ]
        else:
            converted = [[_remove_na(g)] for g in data if _is_numeric(g)]
        if labels and len(labels) != len(converted):
            raise ValueError(
                "The number of labels does not match the length of the list."
            )
    ...
    if kind == "kde":
        try:
            ...
            if len(distinct_values) == 1:
                ...
                warnings.warn(
                    "The data contains a group with a single distinct value ({}) "
