"""From https://github.com/neuralhydrology/neuralhydrology/blob/master/neuralhydrology/modelzoo/head.py"""
...
def get_head(n_in: int, n_out: int, cfg_head="regression", output_activation="relu") -> nn.Module:
    ...
    cfg_head :str
        The run configuration for head function.
    output_activation: str
        the activation for regression only
    Returns
    -------
    nn.Module
        The model head, as specified in the run configuration.
    """
    if cfg_head.lower() == "regression":
        head = Regression(n_in=n_in, n_out=n_out, activation=output_activation)
    elif cfg_head.lower() == "gmm":
        head = GMM(n_in=n_in, n_out=n_out)
    elif cfg_head.lower() == "umal":
        head = UMAL(n_in=n_in, n_out=n_out)
    elif cfg_head.lower() == "cmal":
        head = CMAL(n_in=n_in, n_out=n_out)
    elif cfg_head.lower() == "":
        raise ValueError(f"No 'head' specified in the config")
    else:
        raise NotImplementedError(f"{cfg_head} not implemented or not linked in `get_head()`")
    ...
    def __init__(self, n_in: int, n_out: int, activation: str = "linear"):
        ...
        if activation != "linear":
            if activation.lower() == "relu":
                ...
                hydro_logger.warning(f"## WARNING: Ignored output activation {activation} and used 'linear' instead.")
